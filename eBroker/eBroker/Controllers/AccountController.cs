...
namespace eBroker.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            {
                string message;
                eBroker.BL.BL x = new BL.BL();
                var u = x.UserLogin(model.UserName, model.Password, out message);
                string bankName = "";
                if (u != null)
                {
                    ...
                    var bankDetails = dc.Bank.Where(y => y.Id == u.CompanyID).FirstOrDefault();
                    if (bankDetails != null)
                        bankName = bankDetails.BankName;
                }
                int expiredPolicyCounter = x.expiredPolicy(bankName);
                if (u == null)
                {
                ...
                {
                    //---- Login user
                    SetUserLogedIn(u, false, expiredPolicyCounter);
                    //ViewData["UserData"] = u;
                    Session["ID"] = u.Id;
        ...
        private void SetUserLogedIn(UserModel user, bool rememberMe, int expiredPolicyCounter)
        {
            ...
            serializeModel.Names = user.Names;
            ...
            claims.Add(new Claim("userData", value));
            claims.Add(new Claim("expiredPolicies", expiredPolicyCounter.ToString()));
            ...
            {
                AllowRefresh = new bool?(true),
                IsPersistent = rememberMe,
                ExpiresUtc = new DateTimeOffset?(DateTime.UtcNow.AddDays(7.0))
